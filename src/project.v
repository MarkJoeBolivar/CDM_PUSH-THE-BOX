...
module tt_um_AbAdA_2048 (
  ...
  reg sync_left_0, sync_left_1, sync_right_0, sync_right_1;
  ...
  always @(posedge clk) begin
    if (sys_rst) begin
      ...
      sync_up_0    <= raw_up    | btn_up_in;
      sync_up_1    <= sync_up_0;

      sync_down_0  <= raw_down  | btn_down_in;
      sync_down_1  <= sync_down_0;

      sync_left_0  <= raw_left  | btn_left_in;
      sync_left_1  <= sync_left_0;

      sync_right_0 <= raw_right | btn_right_in;
      sync_right_1 <= sync_right_0;

      sync_start_0 <= raw_start | btn_start_in;
      sync_start_1 <= sync_start_0;
    end
  end
  ...
  wire [3:0] t_x = p_x + d_x;
  wire [3:0] t_y = p_y + d_y;
  wire [3:0] b_x = t_x + d_x;
  wire [3:0] b_y = t_y + d_y;
  ...
  wire b0_is_wall_check, b1_is_wall_check, b2_is_wall_check;

  level_map m_b0_check(
      .level(current_level),
      .x(b0_x),
      .y(b0_y),
      .is_wall(b0_is_wall_check),
      .is_goal(b0_on_goal)
  );

  level_map m_b1_check(
      .level(current_level),
      .x(b1_x),
      .y(b1_y),
      .is_wall(b1_is_wall_check),
      .is_goal(b1_on_goal)
  );

  level_map m_b2_check(
      .level(current_level),
      .x(b2_x),
      .y(b2_y),
      .is_wall(b2_is_wall_check),
      .is_goal(b2_on_goal)
  );
  ...
  level_map render_map (
      ...
      .x(tile_col),
      .y(tile_row),
      .is_wall(render_is_wall),
      .is_goal(render_is_goal)
  );
  ...
  wire is_star = (abs_dx + abs_dy < 12) ||
                 (abs_dx < 2 && abs_dy < 14) ||
                 (abs_dy < 2 && abs_dx < 14);

  wire is_box_shape = (r_local_x > 2 && r_local_x < 29 &&
                       r_local_y > 2 && r_local_y < 29);

  wire is_border = (r_local_x == 5'd0 ||
                    r_local_y == 5'd0 ||
                    r_local_x == 5'd31 ||
                    r_local_y == 5'd31);
  ...
  always @(posedge clk) begin
    if (r_in_grid) begin
      if (is_border)
        r_final_color <= C_GRID_LINE;
      else if (r_player && is_circle)
        r_final_color <= C_BLUE;
      else if (r_any_box && is_box_shape)
        r_final_color <= r_goal ? C_GOLD : C_BROWN;
      else if (r_wall)
        r_final_color <= C_GRAY;
      else if (r_goal && is_star)
        r_final_color <= C_GREEN;
      else
